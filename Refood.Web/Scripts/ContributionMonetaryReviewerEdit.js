...
Web.contributionMonetaryReviewerViewModel = function (moduleId, resx) {
    ...
    var init = function () {
        ...
        var contributionMonetaryReviewerId = qs["tid"];
        if (contributionMonetaryReviewerId) {
            getContributionMonetaryReviewer(contributionMonetaryReviewerId);
        }
        //getUserList();
    ...
    var getContributionMonetaryReviewer = function (contributionMonetaryReviewerId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + contributionMonetaryReviewerId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var save = function () {
        ...
        if (contributionMonetaryReviewer.contributionMonetaryReviewerId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += contributionMonetaryReviewer.contributionMonetaryReviewerId;
        }
        else {
            restUrl += 0;
        }
        ...
        var jqXHR = $.ajax({
            ...
            data: JSON.stringify(contributionMonetaryReviewer),
            dataType: "json"
        }).done(function (data) {
            console.log(data);
