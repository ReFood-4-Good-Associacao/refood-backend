...
Web.contributionTypeViewModel = function (moduleId, resx) {
    ...
    var init = function () {
        ...
        var contributionTypeId = qs["tid"];
        if (contributionTypeId) {
            getContributionType(contributionTypeId);
        }
        //getUserList();
    ...
    var getContributionType = function (contributionTypeId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + contributionTypeId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var save = function () {
        ...
        if (contributionType.contributionTypeId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += contributionType.contributionTypeId;
        }
        else {
            restUrl += 0;
        }
        ...
        var jqXHR = $.ajax({
            ...
            data: JSON.stringify(contributionType),
            dataType: "json"
        }).done(function (data) {
            console.log(data);
