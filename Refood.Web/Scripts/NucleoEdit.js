...
Web.nucleoViewModel = function (moduleId, resx) {
    ...
    var getNucleo = function (nucleoId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + nucleoId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var load = function (data) {
        ...
        openingDate(serverDateTimeStringToJavascriptDate(data.openingDate));
    ...
    var save = function () {
        ...
        var nucleo = {
            ...
            photo: photo(),
            addressId: addressId(),
            openingDate: dateToStringServerFormat(openingDate()),
        ...
        if (nucleo.nucleoId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += nucleo.nucleoId;
        }
        else {
            restUrl += 0;
        }
