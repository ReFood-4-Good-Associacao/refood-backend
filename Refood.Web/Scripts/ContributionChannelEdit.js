...
Web.contributionChannelViewModel = function (moduleId, resx) {
    ...
    var init = function () {
        ...
        var contributionChannelId = qs["tid"];
        if (contributionChannelId) {
            getContributionChannel(contributionChannelId);
        }
        //getUserList();
    ...
    var getContributionChannel = function (contributionChannelId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + contributionChannelId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var save = function () {
        ...
        if (contributionChannel.contributionChannelId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += contributionChannel.contributionChannelId;
        }
        else {
            restUrl += 0;
        }
        ...
        var jqXHR = $.ajax({
            ...
            data: JSON.stringify(contributionChannel),
            dataType: "json"
        }).done(function (data) {
            console.log(data);
