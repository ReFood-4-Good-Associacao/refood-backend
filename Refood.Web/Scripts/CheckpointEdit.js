...
Web.checkpointViewModel = function (moduleId, resx) {
    ...
    var getCheckpoint = function (checkpointId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + checkpointId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var load = function (data) {
        ...
        minimumTime(serverDateTimeStringToJavascriptDate(data.minimumTime));
        maximumTime(serverDateTimeStringToJavascriptDate(data.maximumTime));
    ...
    var save = function () {
        ...
        var checkpoint = {
            ...
            minimumTime: dateToStringServerFormat(minimumTime()),
            maximumTime: dateToStringServerFormat(maximumTime()),
            nucleoId: nucleoId(),
        ...
        if (checkpoint.checkpointId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += checkpoint.checkpointId;
        }
        else {
            restUrl += 0;
        }
