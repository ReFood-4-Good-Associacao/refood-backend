...
Web.energySourceViewModel = function (moduleId, resx) {
    ...
    var init = function () {
        ...
        var energySourceId = qs["tid"];
        if (energySourceId) {
            getEnergySource(energySourceId);
        }
        //getUserList();
    ...
    var getEnergySource = function (energySourceId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + energySourceId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var save = function () {
        ...
        if (energySource.energySourceId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += energySource.energySourceId;
        }
        else {
            restUrl += 0;
        }
        ...
        var jqXHR = $.ajax({
            ...
            data: JSON.stringify(energySource),
            dataType: "json"
        }).done(function (data) {
            console.log(data);
