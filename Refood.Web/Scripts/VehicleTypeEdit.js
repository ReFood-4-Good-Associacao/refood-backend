...
Web.vehicleTypeViewModel = function (moduleId, resx) {
    ...
    var getVehicleType = function (vehicleTypeId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + vehicleTypeId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var save = function () {
        ...
        if (vehicleType.vehicleTypeId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += vehicleType.vehicleTypeId;
        }
        else {
            restUrl += 0;
        }
