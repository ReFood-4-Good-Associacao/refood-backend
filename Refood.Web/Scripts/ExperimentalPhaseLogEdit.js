...
Web.experimentalPhaseLogViewModel = function (moduleId, resx) {
    ...
    var init = function () {
        ...
        var experimentalPhaseLogId = qs["tid"];
        if (experimentalPhaseLogId) {
            getExperimentalPhaseLog(experimentalPhaseLogId);
        }
        //getUserList();
    ...
    var getExperimentalPhaseLog = function (experimentalPhaseLogId) {
        isLoading(true);

        var restUrl = baseUrl + "Get/" + experimentalPhaseLogId;
        var jqXHR = $.ajax({
            url: restUrl,
            dataType: "json"
        }).done(function (data) {
    ...
    var load = function (data) {
        ...
        logDate(serverDateTimeStringToJavascriptDate(data.logDate));
        task(data.task);
    ...
    var save = function () {
        ...
        var experimentalPhaseLog = {
            ...
            nucleoId: nucleoId(),
            logDate: dateToStringServerFormat(logDate()),
            task: task(),
        ...
        if (experimentalPhaseLog.experimentalPhaseLogId > 0) {
            // ajaxMethod = "PATCH";
            restUrl += experimentalPhaseLog.experimentalPhaseLogId;
        }
        else {
            restUrl += 0;
        }
        ...
        var jqXHR = $.ajax({
            ...
            data: JSON.stringify(experimentalPhaseLog),
            dataType: "json"
        }).done(function (data) {
            console.log(data);
